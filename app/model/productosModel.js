...
Productos.getAllProductosApp = function (app,result) {
    ...
    sql.query("Select p.id,p.id_negocio,p.descripcion,p.precio,p.foto,p.fchalta "
    +", IF(WEEK(current_date()) -WEEK(p.fchalta), 'hace mas de una semana', 'esta semana')"
    +" as tiempo, c.descripcion comercio"
    +" from productos p, comercios c where p.app=1 and  p.fchcad is null"  +" and p.id_negocio = c.id ",[app], function (err, res) {
